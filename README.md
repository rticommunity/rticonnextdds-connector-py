...
For now, the responsibility of protecting calls to the *Connector* is left to you. (In future, thread safety may be added in the native layer.)
In Python, you will have to protect the calls to *Connector* if you are using different threads. For an example, see [Protecting calls to the Connector library](https://github.com/rticommunity/rticonnextdds-connector-py#protecting-calls-to-the-connector-library) below.
...
For more information on the threading Python packages, see the [Python documentation](https://docs.python.org/2/library/threading.html).
...
look at the [ShapeExample.xml](examples/python/ShapeExample.xml) file included in this examples directory.
