...
class TestMetadata:
  ...
  def test_get_disposed_key_value(self, one_use_output, one_use_input):
    ...
    # Any other fields should be None
    assert sample.get_number("x") is None
    assert sample.get_boolean("z") is None
    assert sample["x"] is None
    assert sample["z"] is None
    assert sample["shapesize"] is None
  ...
  def test_get_disposed_key_values_nested_keys(self, one_use_connector):
    the_input = connector.get_input("MySubscriber::MyNestedKeyedSquareReader")
    the_output = connector.get_output("MyPublisher::MyNestedKeyedSquareWriter")
    # Wait for discovery
    the_input.wait_for_publications(5000)
    the_output.wait_for_subscriptions(5000)
    # Set the sample
    ...
    the_output.instance["keyed_toplevel_member"] = 1
    ...
    sample = send_data(the_output, the_input, action="dispose")
    assert sample.info["is_valid"] == False
    assert sample.info["instance_state"] == "NOT_ALIVE_DISPOSED"
    ...
    assert sample["keyed_nested_member.x"] is None
    assert sample["unkeyed_toplevel_member"] is None
    assert sample["keyed_toplevel_member"] == 1
    assert sample["unkeyed_shape"] is None
    ...
    assert sample.get_number("keyed_nested_member.x") is None
    assert sample.get_number("keyed_toplevel_member") == 1
    assert sample.get_number("unkeyed_toplevel_member") is None
    # Obtain the entire sample as a dictionary (should only contain keys)
    expected_dictionary = {
        "keyed_shape": {
            "color": "Black",
            "x": 2,
